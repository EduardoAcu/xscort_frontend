...
export default function BotonesContacto({ perfil }) {
  ...
  const telegramUrl = perfil.telegram_contacto
    ? `https://t.me/${perfil.telegram_contacto.replace("@", "")}`
    : null;

  // Si no hay ninguno, no mostramos nada
  ...
  return (
    // CAMBIO 1: Quitamos 'flex-col' y dejamos solo 'flex-row' para que estén lado a lado
    <div className="flex flex-row gap-3 w-full max-w-5xl mx-auto mt-6 font-montserrat">
      ...
      {whatsappUrl ? (
        <a
          ...
          className="
            ...
            h-12 rounded-xl /* CAMBIO 2: Más alto y redondeado */
            border border-[#25D366]/30 bg-[#25D366]/10
            text-[#25D366] text-sm font-bold uppercase tracking-widest /* CAMBIO 3: Letra más grande */
        ...
        /* Estado deshabilitado visual si quieres mantener simetría (opcional) */
        telegramUrl && <div className="hidden"></div>
      )}
      ...
      {telegramUrl && (
        <a
          ...
          className="
            flex-1 flex items-center justify-center gap-2
            h-12 rounded-xl
            border border-[#0088cc]/30 bg-[#0088cc]/10
            text-[#0088cc] text-sm font-bold uppercase tracking-widest
